...
\subsection{Overall Software Design}
	The overall software design is shown in Figure~\ref{fig:softwareblockdiagram} and was written using Matlab (Mathworks, Natick, MA). The input to the software is a navigator image, which was captured using the DVI framegrabber and the Matlab wrapper class for the Java interface, which contained a method that allowed simple capture of the frame. Since the navigator image is updated with new data with every patient heartbeat (typically lower than 120 heartbeats per minute), a frame sampling rate of 10 fps will be sufficient to capture all changes in diaphragm position. The diaphragm location will be identified and if different from the previous position (stored), then the new position will be used to update the videogame image.
